...
def compute_nme(preds, targets):
    """Compute NME (Normalized Mean Error)"""
    ...
    # khoảng cách từng landmark
    diff = torch.norm(preds - targets, dim=2)  # (B, N)

    # trung bình mỗi ảnh
    mean_error = diff.mean(dim=1)  # (B,)

    # chuẩn hóa bằng bounding box
